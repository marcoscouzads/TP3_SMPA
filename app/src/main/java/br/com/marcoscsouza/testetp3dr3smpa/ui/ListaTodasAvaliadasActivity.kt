...
import br.com.marcoscsouza.testetp3dr3smpa.databinding.ActivityListaTodasAvaliadasBinding
...
class ListaTodasAvaliadasActivity : AppCompatActivity() {
    private val binding by lazy {
        ActivityListaTodasAvaliadasBinding.inflate(layoutInflater)
    }
    private val firebaseAuth = Firebase.auth
    private val adapter = AvaliadosAdapter(this)
    override fun onCreate(savedInstanceState: Bundle?) {

        super.onCreate(savedInstanceState)
        setContentView(binding.root)

        val rv = binding.rvTodasAvaliadas
        rv.adapter = adapter
