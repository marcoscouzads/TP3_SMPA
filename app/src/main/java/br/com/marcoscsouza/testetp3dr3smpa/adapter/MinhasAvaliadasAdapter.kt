...
class MinhasAvaliadasAdapter(
    private val context: Context,
    avaliados: List<Avaliar> = emptyList()
) : RecyclerView.Adapter<MinhasAvaliadasAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: AvaliarItemBinding) :
        ...
        fun vincula(avaliar: Avaliar) {
            ...
            primeiraPergunta.text =
                "Local possui bom atendimento?   ${converterResposta(avaliar.pergunta1)}"

            val segundaPergunta = binding.avaliarItemPergunta2
            segundaPergunta.text =
                "Local possui boa higienização?   ${converterResposta(avaliar.pergunta2)}"

            val terceiraPergunta = binding.avaliarItemPergunta3
            terceiraPergunta.text =
                "Local possui comida de qualidade?   ${converterResposta(avaliar.pergunta3)}"

            val quartaPergunta = binding.avaliarItemPergunta4
            quartaPergunta.text =
                "Local possui preço justo?   ${converterResposta(avaliar.pergunta4)}"

            val quintaPergunta = binding.avaliarItemPergunta5
            quintaPergunta.text =
                "Local possui um ambiente agradável?   ${converterResposta(avaliar.pergunta5)}"

            val sextaPergunta = binding.avaliarItemPergunta6
            sextaPergunta.text =
                "Local possui uma cozinha limpa?   ${converterResposta(avaliar.pergunta6)}"
        }
    }
